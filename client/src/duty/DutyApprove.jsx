...
function DutyApprove() {
                ...
                {requestList.map((request) => (
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                <span className="mt-1 text-4xl"></span>{request.examName}
                            </Typography>
                        </CardHeader>
                        <CardBody className="p-0">
                            <ul className="flex flex-col gap-4" style={{ paddingTop: "30px" }}>
                                <li className="flex items-center gap-4">
                                    <span className="rounded-full border border-white/20 bg-white/20 p-1">
                                        <UsersIcon />
                                    </span>
                                    <Typography className="font-normal">{request.numberOfTeachers} Invigilators Required</Typography>
                                </li>
                                <li className="flex items-center gap-4">
                                    <span className="rounded-full border border-white/20 bg-white/20 p-1">
                                        <CalendarIcon />
                                    </span>
                                    <Typography className="font-normal">{request.date}</Typography>
                                </li>
                                <li className="flex items-center gap-4">
                                    <span className="rounded-full border border-white/20 bg-white/20 p-1">
                                        <ClockIcon />
                                    </span>
                                    <Typography className="font-normal">{request.time}</Typography>
                                </li>
                            </ul>
                        </CardBody>
    ...
    const pendingRequests = requests.filter(request => request.status === 0 && request.department === department)
