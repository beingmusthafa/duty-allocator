...
function StaffApprove() {
  ...
  const fetchRequestData = async () => {
    try {
      const response = await axios.get(
        `http://localhost:3000/duty/${requestId}`
      );
  ...
  const handleApprove = () => {
    ...
    axios
      .post("http://localhost:3000/staff/approve", approvalData)
      .then((response) => {
        const approvalId = response.data.approvalId;
        console.log("Request approved:", approvalId);
        const status = 1;
        axios
          .put("http://localhost:3000/duty/status", { status, requestId })
          .then((res) => {
              ...
              >
                ...
                <CardBody className="flex-grow items-center justify-center text-center">
                  <Typography
                    variant="h6"
                    color="gray"
                    className="mb-4 uppercase text-lg"
                  >
                    Exam Details
                  </Typography>
                  <Typography
            ...
            {request && (
              ...
              >
                ...
                <CardFooter className="mt-4 p-0">
                  {selectedStaff.length < request.numberOfTeachers && (
                    <p className="font-semibold text-center mb-4 text-sm w-44 mx-auto">
                      Select{" "}
                  ...
                  <Button
                    size="lg"
                    disabled={selectedStaff.length < request.numberOfTeachers}
                    color="white"
